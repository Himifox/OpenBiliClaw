...
交接时 Core 先做聚合兴趣与敏感主题门禁，再把追踪、Phase 1 语义和 Phase 2 表达分层；
Core 可在内部预览并排序最多 3 条候选，但 NEKO 适配层只把排名第 1 条交给
Phase 1，OBC 在总候选集中最多占 1 个槽位；Phase 2 只看到最终一条的标题、主题、
摘要和选题动机。嵌入 NEKO 时 Core 使用 `surface_copy_mode="lazy"`，后台不会生成
重复推荐台词，内容质量、兴趣匹配、时效与敏感门禁仍全部由 OBC 完成。
内嵌维护只保留 30 条有效容量，低于 4 条时单批最多补 10 条，并以每日 100k 输入、
20k 输出 Token 的持久预算限制 OBC 后台调用；lazy Core 会在后台启动前自动安装这套
门禁。NEKO Phase 1/2 的实际 Token 写入同一用量台账但不挤占后台额度，容量不会在
启动时被追满。
摘要可靠度由 Discovery 在原有同一次评估中独立评分；摘要存在但空泛或不可验证时同样
不会交给 Phase 1，不会为此增加一次模型调用。
完整画像、原始行为、内容链接和内部投递身份不会进入模型 Prompt。

...
- 插件是 OpenBiliClaw 的浏览器采集层：NEKO/Core 未运行时，行为事件仍按稳定 `event_id` 写入 `chrome.storage.local`；live/inflight/parked 共用 1000 条容量，保留 30 天，按最旧事件淘汰。Core 恢复后以每批 100 条按 `inflight → parked → live` 自动补传；该路径不要求启用 NEKO 插件系统或 MCP
...
canonical pool → aggregate signals + sensitive gate → Tracking / Semantics / Policy
               ...
               → Phase 1/2 provider usage → embedded.proactive.* ledger rows
...
Recommendation, 30k Soul) plus a 20k/day background output ceiling. Lazy Core installs
this preset before runtime construction and rejects unbounded overrides. Interactive and
host Phase usage stays observable in the same ledger without reducing background
remaining capacity. Capacity is not a startup fill target.

...
| Awareness with confusions (`soul.awareness_confusions`) | Production cognition cycle | `compact-v1` remains default after the 2026-08-06 task gate; opt-in `bounded-v2` uses `build_bounded_awareness_with_confusions_prompt()` | bounded-v2: bounded soul baseline + 8 baseline interests + recent 8 awareness + active 8 insights + whole identity-safe activity envelopes; author/activity/matched interests remain nested, while IDs/URLs/full creator inventory stay in a private manifest | Bounded stable subset | Yes |

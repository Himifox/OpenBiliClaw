...
当前模块已经实现 canonical identity / typed contracts、capability router、local-first sync service、SQLite DAO 边界、首个生产实现 `BilibiliNativeSaveAdapter`、平台中立 HTTP API / runtime 注册，以及桌面 Web、移动 Web 的保存与同步界面。浏览器连接器不再承载保存列表或设置，只在 background 执行六平台 durable 原生保存任务。旧端点仍只做本地 B 站兼容保存，不会因本次 wiring 自动修改平台账号。
...
| 两个 Web 保存界面 + CLI 配置可见 | ✅ | 移动 Web 与桌面 Web 把「稍后再看 / 收藏 / 历史记录」收进单一内容库入口；两个保存子项复用原有列表与状态实现，不复制 API 或平台路由矩阵。推荐卡只对本地保存做 optimistic update；状态与并发 fence 按 `list_kind:item_key` 隔离，保存列表由后端 `sync_status / sync_task_id / resolved_target / error_code` 驱动，并以有界 timeout、generation fence 和 durable task ownership 防止迟到响应覆盖新 mutation。`extension_required` 只用于提示需要在线且已登录的平台连接器；连接器本身不渲染列表、保存按钮或同步配置。CLI 仅由 `config-show` 展示自动同步开关，不提供保存 / 同步动作。 |
| 本地移除历史与恢复（issue #112） | ✅ | `remove_saved_membership()` 删除 normalized membership 时，在同一 SQLite 事务写入 `saved_item_removals` metadata snapshot；不会反向删除平台保存，也不会复制 native task ledger。桌面与移动 Web 的「最近移除」固定只投影 30 天窗口；物理快照在数据库初始化和后续 membership 删除时 opportunistic prune，不宣称存在独立周期任务保证到点即删。同一 canonical `item_key` 的 favorite / watch_later / dismiss / dislike 以 `contexts[]` 在一张卡中返回，各 context 只保留自己的最新事实；favorite 与 watch_later 分别按对应 `list_kind + item_key` 计算 restored，用户可通过原有 `POST /api/saved/{list_kind}` 独立恢复。顶层 `context/restored` 只保留为旧客户端兼容投影，新界面不得用它代替完整 contexts。 |
| 保存卡片反馈操作（issue #111） | ✅（桌面 Web + 移动 Web；连接器与 CLI 无卡片 UI） | 桌面 Web 与移动 Web 的「稍后再看 / 我的收藏」列表卡片复用推荐卡反馈操作条。稍后再看 / 收藏走共享 `handleCardAction`；喜欢 / 不感兴趣 / 忽略 / 聊一聊因 saved 项无 `recommendation_id`，改走内容维度 `/api/events`。两端统一提供低调操作条与跨列表保存 toggle，并分别完成真实浏览器验证。浏览器连接器只执行 durable 平台任务，不提供保存卡、反馈条或同步按钮。 |

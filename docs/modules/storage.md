...
| 持久化已看去重 | ✅ | `seen_items(item_key)` 保存所有已知 `view / favorite / like / coin` 的 canonical `source_platform:content_id`，B 站同时向旧调用方暴露 raw BVID。单条 `insert_event()` 与批量 `insert_events_batch()` 都通过各自独立短连接在事件事务内同步 upsert 账本；初始化会按 `seen_items_backfill_state` 增量回填旧事件。可换、raw、评估、平台库存与 delight 打分/计数/出队路径读取这份无界账本，`get_recent_viewed_*` 仅作为兼容别名，不再代表“最近 N 条”。`mark_delight_seen()` 供真正展示惊喜卡的可见宿主执行“× / 看过了”时先写 canonical ledger、再消费惊喜状态。 |
...
- 平台 adapter、platform-neutral HTTP API，以及桌面 Web / 移动 Web 的保存与同步 UI
  已接入同一 normalized store。浏览器连接器不渲染保存列表，只执行六平台 extension-backed adapter 的 durable 任务；Bilibili 保持 direct adapter，六平台已注册 extension-backed adapter；

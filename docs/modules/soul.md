...
| Awareness bounded-v2 envelope | ✅ | 仅供 `soul.awareness_confusions` 显式启用。连续且同 content identity、session bucket、author identity 的事件组成不可拆 envelope；标题、作者、行为计数、摘要和最多 2 个本 envelope 匹配兴趣一起投影，作者和兴趣绝不脱离事件单独抽样。全局创作者清单、URL、数据库 ID 和 tracking 不进 prompt。模型只能引用 `E001` 形式的调用期 ref，Core 经私有 manifest 严格映射回该 envelope 的全部 event IDs；未知 ref 丢弃，整批无有效引用则失败关闭 |
...
| CognitionCycle 游标增量取数 | ✅ | 觉察按 `last_awareness_event_id` 读取 `id > 水位` 的最老连续行。legacy/compact 保持既有批处理与 lookback；bounded-v2 按完整 envelope 装入 24k 目标、每周期默认最多 2 次。每个请求前持久化 `{after, through, manifest_digest}`，重启重试时映射变化即失败；仅当模型响应、引用校验和持久化全部成功后才推进水位。积压达到 900 只警告并留待下轮，绝不跳到最新窗口，因此可以收缩 prompt，但不能丢事件或把下一阶段作者错配到另一批。洞察仍按 `last_insight_awareness_index` 只读新觉察 |
...
| SoulEngine.record_immediate_feedback_cognition() | ✅ | 单条 `dislike/comment` 可即时写入结构化 cognition card，供 NEKO 与画像页展示 |
...
| 可见宿主「聊聊口味」长列表与证据展示 | ✅ | `/web`、`/m/` 与 NEKO 共享 durable turn 与安全 renderer；历史继续使用兼容名 `session=popup`。浏览器连接器不渲染确认卡、聊天或画像 |
...
| Cognition updates | ✅ | 在反馈刷新和聊天学习后生成结构化 cognition card，供 NEKO 主动提示与画像页展开；连接器接收事件不确认已展示 |
...
- `GET /api/profile-summary` 返回 `speculative_avoidances`，供移动 Web、桌面 Web 和 NEKO 画像界面展示。

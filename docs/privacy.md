...
| 本地配置、离线队列与 UI 状态 | 后端地址、已关闭提示、插件设置、缓存的后端配置、任务状态、待续传行为事件，以及最后一次成功读取的已初始化画像摘要 | 保持插件连接、本地偏好和界面状态；后端离线时继续记录行为并只读展示最近画像 |
...
插件会使用浏览器的本地扩展存储保存设置、连接状态、缓存配置和 UI 状态。后端不可达时，待续传行为事件最多保留 1000 条、最长保留 30 天；恢复连接后按先进先出顺序发送到用户配置的后端，并在后端确认持久接收后删除。插件还会按后端地址保存最后一次成功读取且已初始化的画像摘要，最长保留 30 天，仅用于离线只读展示；后端明确返回未初始化、后端地址变化、快照过期或 schema 不兼容时不会继续展示旧快照。这些缓存不保存 Cookie、API Key 或设备密钥。OpenBiliClaw 后端会在用户本机或自托管环境中保存配置文件、SQLite 数据库、日志和画像文件。
...
| `storage` | 保存插件设置、本地 UI 状态、后端连接信息、最长 30 天的待续传行为事件和离线只读画像摘要 |

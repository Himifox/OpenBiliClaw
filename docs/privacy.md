...
OpenBiliClaw 是一个本地优先的跨平台内容发现 AI Agent。浏览器插件是轻量连接器：在用户访问受支持内容平台时，采集用户授权范围内的浏览、互动和内容信号，向用户自己配置的 OpenBiliClaw 后端同步身份与行为，并执行后端下发的只读来源任务。推荐、聊天、画像和主动消息由 NEKO、桌面 Web 或移动 Web 等真实可见客户端展示。
...
| 个人通讯 | 用户在 OpenBiliClaw 主应用中主动输入的消息；以及用户在受支持平台上**成功提交**的评论正文与 B 站弹幕正文（提交成功后经网络层采集，仅送本机后端） | 与用户配置的本地后端聊天接口交互，帮助查看画像、推荐和设置；用户亲手写的评论 / 弹幕是最强的兴趣表达之一，用于更准确地构建兴趣画像 |
| 本地配置与 UI 状态 | 后端地址、设备配对、已关闭提示、来源状态摘要、任务状态 | 保持插件连接、本地偏好和界面状态 |
...
嵌入 NEKO 的主动推荐采用失败关闭的语义交接：OpenBiliClaw 在本机先完成去重、时效、
敏感、内容质量和个性化判断，只交付一条有界语义候选给 Phase 1。精确评估缓存的键为
不可逆摘要，值只含验证后的结构化分数/分类；标题、正文、画像、Prompt 以及带逐字正文
证据的结果不会写入该缓存。缓存最多保留 30 天和 20,000 条，可随本地数据库一起清除。
摘要可靠度与内容质量、相关性在同一次本地配置模型调用中评估；该诊断分量不会进入
NEKO Phase 1 Prompt，缺失或低于门槛时直接不交接候选。

NEKO 托管模式最多维护 30 条有效候选的逻辑窗口，低于 4 条才按最多 10 条的小批次
补货；每日 OBC 后台输入最多 100k Tokens。预算恢复只查询 caller、日期和 Token 数量，
不会为了限额另存 Prompt、画像、行为正文或模型响应。
...
- 让本地后端执行多平台初始化画像、内容发现和任务调度。
- 保存插件设置、连接状态和用户明确关闭过的提示。

这些用途均围绕插件的单一用途：作为浏览器与用户自己的 OpenBiliClaw 后端之间的连接、同步与任务执行层。
...
当用户显式启用 Awareness `bounded-v2` 时，本地完整事件仍保存在 SQLite，但发送给模型的是有界语义投影：同一内容、同一会话、同一作者的连续行为与其本地匹配兴趣保持在一个不可拆 envelope 中；URL、事件 ID、内容/作者内部 key、完整创作者清单和原始 tracking 不发送。调用期 `E001` 引用仅由本地私有 manifest 映射回真实事件，未知或错配引用会失败关闭，不能靠标题猜测身份。

嵌入 NEKO 的主动推荐交接不会把完整画像、原始浏览记录、收藏/稍后看条目、行为次数、
推荐推理过程或自由文本推荐表达放入 NEKO Prompt。Core 只交接有界内容语义、聚合兴趣
代码和敏感门禁结果；最近三条用户消息若用于敏感主题的明确上下文判断，仅在当前进程
内存中做确定性匹配，不持久化也不发送给模型。健康、金融、政治和宗教兴趣不能仅凭
浏览画像触发主动话题；诊疗、投资买卖、政治劝服和信仰推断始终过滤。

## 本地存储与保留

插件会使用浏览器的本地扩展存储保存后端端点、设备配对会话、连接/来源状态摘要、outbox 和 UI 状态；不会缓存完整后端业务配置。OpenBiliClaw 后端会在用户本机或自托管环境中保存配置文件、SQLite 数据库、日志和画像文件。
...
插件请求的权限用于完成浏览器连接、行为同步和平台任务，不用于广告、售卖数据或无关追踪：
...
| `sidePanel` | 提供连接状态、当前来源、身份同步和主应用入口 |
| `storage` | 保存后端连接、远程短会话、离线事件 outbox 和最近同步状态 |
| 主机权限 | 限定在 Bilibili、小红书、抖音、YouTube、X、知乎、Reddit、Linux.do、Bangumi、V2EX、微博以及本机 OpenBiliClaw 后端之间处理必要数据 |

`http://127.0.0.1/*` 和 `http://localhost/*` 用于连接用户自己的本机 OpenBiliClaw 后端。内容采集脚本只声明在受支持内容平台上运行；发布包不声明 `http://*/*`、`https://*/*` 或 `<all_urls>` 这类所有网站权限。

插件不再请求 `notifications` 权限，也不会在后台轮询推荐/认知通知后把未展示内容确认成已送达；主动内容只有在 NEKO 或其它真实可见客户端成功展示后才记录交付。
